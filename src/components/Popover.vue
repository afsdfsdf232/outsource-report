    ...
    <div class="popover-item" v-for="item in list" :key="item.title">
      <el-popover :width="230" placement="left" popper-class="popper-class" trigger="hover">
        ...
        <div class="hidden-img" style="width: 200px">
          <img v-if="qrcodeImages[item.link]" style="width: 200px" :src="qrcodeImages[item.link]" alt="" />
          <span v-else>请在后台配置二维码</span>
        </div>
      </el-popover>
    </div>
...
export default defineComponent({
  setup() {
    ...
    const qrcodeImages = computed(() => store.state.configInfo)
    const backTop = () => {
      goTop(5)
    }
    return {
